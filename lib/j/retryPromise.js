  ...
  const retryAttempt = () => {
    ...
    return action()
      .then(function (resource) {
        retryHistory.set(numberOfAttempts, resource)
        console.log(`Retry success: ${retryHistory}`)
        return { resource, retryHistory }
      })
      .catch(function (error) {
        if (numberOfAttempts === maxAttempts) {
          console.log(`Retry failed: ${retryHistory}`)
          return Promise.reject({ error, retryHistory })
        }

        // Initiate new attempt
        numberOfAttempts++

        retryHistory.set(numberOfAttempts, error)
